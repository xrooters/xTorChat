...
{
    return @"Please enter the Username and click done to add new friend";
}
...
{
    if ([self checkFields]) {
        NSString * newBuddyAccountName = [[[NSString stringWithFormat:@"%@@%@", self.accountNameTextField.text, @"xrooters.ro"] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] lowercaseString];
...
- (void)populateFromQRResult:(NSString *)result
{
    ...
    if ([result containsString:@"xmpp:"]) {
        ...
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"xTorChat" message:nil preferredStyle:UIAlertControllerStyleAlert];
        [self presentViewController:alert animated:YES completion:nil];
    }
}
